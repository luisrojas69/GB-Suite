@extends('layouts.app')
@section('title-page', 'Editar Sector: ' . $sector->nombre)
            ...
            <form action="{{ route('produccion.areas.sectores.update', $sector->id) }}" method="POST">
                ...
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-header bg-white py-3 border-bottom">
                        <h6 class="m-0 font-weight-bold text-dark"><i class="fas fa-edit mr-2 text-success"></i> Información General</h6>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-5">
                                    <input type="text" name="codigo_sector" class="form-control form-control-agro @error('codigo_sector') is-invalid @enderror" 
                                           placeholder="Código" value="{{ old('codigo_sector', $sector->codigo_sector) }}">
                                </div>
                                <div class="col-md-7">
                                    <input type="text" name="nombre" class="form-control form-control-agro @error('nombre') is-invalid @enderror" 
                                           placeholder="Nombre del Sector" value="{{ old('nombre', $sector->nombre) }}">
                                </div>
                            </div>
                        ...
                        <div class="agro-input-group mb-3">
                            ...
                            <div class="input-group">
                                <input readonly type="number" step="0.01" name="hectareas_geometria" id="hectareas_geometria" 
                                       class="form-control form-control-agro bg-white font-weight-bold text-primary" 
                                       value="{{ old('hectareas_geometria', $sector->hectareas_geometria) }}">
                        ...
                        <div class="agro-input-group mb-4">
                            ...
                            <textarea name="descripcion" class="form-control form-control-agro" rows="4" 
                                      placeholder="Detalles adicionales sobre el terreno, accesos o limitaciones...">{{ old('descripcion', $sector->descripcion) }}</textarea>
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        @if($sector->geometria_objeto)
            ...
            // Al cargar, vinculamos eventos a las capas cargadas
            geoLayer.eachLayer(layer => {
                layer.on('pm:edit pm:dragend', e => {
                    updateGeometryStats(e.target);
                });
                // Actualizar perímetro inicial
                const length = turf.length(layer.toGeoJSON(), {units: 'kilometers'});
                $('#perimetro-val').text(length.toFixed(2) + ' km');
            });
        @endif
